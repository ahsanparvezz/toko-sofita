...
1. Apa perbedaan antara synchronous request dan asynchronous request?
    Perbedaan antara synchronous request dan asynchronous request terletak pada cara browser menunggu respon dari server. Pada synchronous request, browser akan menunggu hingga server mengirimkan respon sebelum melanjutkan eksekusi kode berikutnya, sehingga seluruh halaman bisa “terhenti” sementara menunggu. Sedangkan pada asynchronous request, browser tetap dapat menjalankan kode lain tanpa harus menunggu server selesai memproses request. Hal ini membuat pengguna tetap bisa berinteraksi dengan halaman, seperti menggulir atau menekan tombol, meskipun data sedang dimuat di latar belakang.
2. Bagaimana AJAX bekerja di Django (alur request–response)?
    AJAX di Django bekerja melalui serangkaian tahapan yang dimulai dari sisi klien. Pertama, JavaScript di browser mengirimkan permintaan HTTP ke server Django tanpa perlu reload halaman. Kemudian Django menerima request ini melalui URL routing yang mengarah ke view tertentu. View akan memproses data, berinteraksi dengan model database jika diperlukan, dan mengembalikan response dalam format JSON atau XML. Data ini kemudian ditangkap oleh callback function di JavaScript yang akan memperbarui bagian tertentu dari halaman menggunakan DOM manipulation. Seluruh proses ini terjadi secara diam-diam di balik layar tanpa mengganggu experience pengguna.
3. Apa keuntungan menggunakan AJAX dibandingkan render biasa di Django?
    Keuntungan menggunakan AJAX dibandingkan render biasa di Django adalah efisiensi dan responsivitas. Dengan AJAX, hanya bagian tertentu dari halaman yang diperbarui, sehingga pengguna tidak perlu menunggu seluruh halaman reload. Ini mengurangi beban jaringan dan membuat interaksi terasa lebih cepat. Selain itu, AJAX memungkinkan komunikasi real-time dengan server, misalnya menampilkan notifikasi, update stok, atau memuat data tambahan tanpa mengganggu tampilan yang sudah ada.
4. Bagaimana cara memastikan keamanan saat menggunakan AJAX untuk fitur Login dan Register di Django?
    Keamanan implementasi AJAX untuk autentikasi memerlukan pendekatan multi-layer. Pertama, selalu gunakan HTTPS untuk mengenkripsi data sensitif seperti password selama transmisi. Kedua, implementasi CSRF protection dengan menyertakan token CSRF di setiap request AJAX melalui header X-CSRFToken. Ketiga, validasi data secara ketat di server side meskipun sudah dilakukan di client side. Keempat, gunakan Django's built-in authentication system dan batasi rate request untuk mencegah brute force attack. Kelima, terapkan proper session management dan hindari exposure informasi sensitif melalui response JSON.
5. Bagaimana AJAX mempengaruhi pengalaman pengguna (User Experience) pada website?    
    AJAX mempengaruhi pengalaman pengguna (User Experience) pada website dengan membuat interaksi lebih cepat dan lancar. Karena halaman tidak harus di-reload seluruhnya, pengguna dapat melihat perubahan secara instan, seperti update tabel, notifikasi, atau login langsung tanpa berpindah halaman. Hal ini membuat website terasa lebih modern, responsif, dan nyaman digunakan, sehingga pengguna cenderung betah berinteraksi lebih lama. Dengan kata lain, AJAX meningkatkan kelancaran interaksi dan mengurangi frustasi akibat loading lama.
